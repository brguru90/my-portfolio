...
class projects extends Component {
    ...
    generate_bg_img = (width, _length = 2) => {
        ...
        const img_height = (multiplier * _length) + 400
        const vw = Math.ceil((img_height) / 1.5)

        const svg_img = (
            <svg width={`${vw}px`} height={`${img_height}px`} viewBox={`0 0 ${vw} ${img_height}`}
                xmlns="http://www.w3.org/2000/svg" version="1.1">
                {/* <rect x="1" y="1" width={width} height={multiplier * _length}
                    fill="none" stroke="blue" stroke-width="1" /> */}

                {/* <path d="M1200,0 C1200,0 200,0 600,400 Q800,600 600,800 T600,1200 T600,1600 C600,1600 400,1800 0,1800 L0,2000 C1400,2000 1200,1600 1200,0 L1200,0"
        fill="none" stroke="red" stroke-width="5"  /> */}
        {/* multiplier * _length --> 1600 */}

                <path
                    className="svg_bg_area"
                    d={
                        `M1200,0 C1200,0 200,0 600,400 Q800,600 600,800 ${sequence}
                        C600,${multiplier * _length} 400,${(multiplier * _length) + 200} 0,${(multiplier * _length) + 200}
                        L0,${(multiplier * _length) + 400}
                        C${(multiplier * _length) - 200},${(multiplier * _length) + 400} ${(multiplier * _length) - 400},${(multiplier * _length)} ${(multiplier * _length) - 400},0
                        L${(multiplier * _length) - 400},0`
                    }
                    fill={bg_color}
                // stroke="red"
                // stroke-width="5"
